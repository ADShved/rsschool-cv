#Andrey Shvedovsky
**Junior Frontend/Fullstack Developer**

[Город] • [Телефон] • [Email] • [Telegram/LinkedIn] • [GitHub Profile Link]

---

### 🚀 About me
A dedicated beginner developer with a focus on writing clean and maintainable code. My priority is continuous development and deep immersion in technology. I am quick to learn new tools, have a good understanding of documentation, and strive to contribute to the creation of high-quality software products.

### 🛠 Skills
*   **Languages:** JavaScript (ES6+), HTML5, CSS3/SASS.
*   **Tools:** **Git**, GitHub, VS Code, npm/yarn.
*   **Methodologies:** BEM, the basics Agile/Scrum.
*   **Version control systems:** Free ownership Git (Conventional Commits, Branching).


### 💼 Work experience / Educational projects
**Training Practice / Self-Study** | *2026 - Present*
* Developed and deployed 3+ educational projects.
* Set up project build automation via GitHub Actions (optional).
* Apply teamwork skills via Pull Requests and Code Review within courses.



### 🌍English language
**Level: [ A2 - Elementary ]**
* I can read technical documentation fluently (MDN, Stack Overflow).
* I have experience watching English-language tutorials and lectures without subtitles.